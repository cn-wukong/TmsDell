...
public class LoginController {
     ...
     * @return
     */
    @PostMapping("/card/destory/now")
    public String cardDestroyXiu(Model model,String cardnum) {
        String sate = "作废";
        List<Card> cards = storageManageService.cardList(cardnum);
        try {
            storageManageService.updateDestroyCard(sate,cards);
            model.addAttribute("message","作废成功");
        }catch (Exception e){
            model.addAttribute("message","作废失败");
            e.printStackTrace();
        }
     ...
     * @return
     */
    @GetMapping("/card/account")
    public String cardDestorySousuo(Model model,String souAccount) {
            List<Card> cards = storageManageService.cardList(souAccount);

            if (cards.isEmpty()){
                String message = "卡号不存在";
                model.addAttribute("cardMessage",message);
            }else{
     ...
     * @return
     */
    @PostMapping("/card/sell/address")
    public String cardSellAdress(Model model,String address,String name,String nameSell,String phone,Integer numbersell){
        String message = "";
        String sate = "已下发至售票点";
        try {
            List<Card> cards = storageManageService.takeCard(numbersell);
            storageManageService.updateCardSell(sate,nameSell,cards);
             message = "下发成功";
     ...
     * @return
     */
    @GetMapping("/card/already/down")
    public String cardSellEnd(Model model){
       List<Card> cardList =  storageManageService.selectCardSellEnd();
       List<Card> cardList1 = storageManageService.selectCardSellEndAll();
        Integer cardSellEnd = cardList1.size();
        model.addAttribute("cardSellEnd",cardSellEnd);
